    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-stone-600 mt-1">
            ...
            {unreadCount > 0 && (
              <span className="ml-2 text-[#C97B4B] font-medium">({unreadCount} unread)</span>
            )}
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-xl border">
          ...
          <h3 className="text-lg font-medium text-stone-900 mb-1">No messages</h3>
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl border transition-all hover:shadow-md ${
                    !msg.isRead ? 'border-l-4 border-l-[#C97B4B]' : ''
                  }`}
                >
                  <div
                  ...
                  >
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 ${
                      msg.isRead ? 'bg-stone-400' : 'bg-[#C97B4B]'
                    }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`font-semibold text-stone-900 ${!msg.isRead ? 'font-bold' : ''}`}>
                          {msg.name}
                        </p>
                        {!msg.isRead && (
                          <span className="w-2 h-2 bg-[#C97B4B] rounded-full flex-shrink-0" />
                        )}
                      </div>
                    ...
                    <div className="hidden md:block text-right flex-shrink-0">
                      <p className="text-xs text-stone-400">{timeAgo(msg.createdAt)}</p>
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-2 text-sm text-stone-700">
                            ...
                            <a href={`mailto:${msg.email}`} className="text-[#1B4965] hover:underline">{msg.email}</a>
                          </div>
                          {msg.phone && (
                            <div className="flex items-center gap-2 text-sm text-stone-700">
                              <Phone className="w-4 h-4 text-stone-400" />
                              <a href={`tel:${msg.phone}`} className="text-[#1B4965] hover:underline">{msg.phone}</a>
                            </div>
                          )}
                        ...
                        {msg.subject && (
                          <div className="space-y-2">
                            <h4 className="text-xs font-bold text-stone-500 uppercase tracking-wider">Subject</h4>
                            <p className="text-sm text-stone-700 font-medium">{msg.subject}</p>
                          </div>
                        )}
                      </div>
