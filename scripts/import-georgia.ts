...
async function undo(batch: string) {
  ...
  const order = [
    ...
    'listings', 'units', 'buildings', 'location_guides', 'users',
  ];
...
async function run() {
  ...
  const stats = {
    users: 0, usersMerged: 0, guides: 0, buildings: 0, units: 0, listings: 0,
    investment: 0, documents: 0, favourites: 0, skipped: 0,
  };
  const notes: string[] = [];
  ...
  for (const p of props) {
    ...
    for (const u of rows) {
      ...
      if (!String(u.id).endsWith('-implied')) await record('units', u.id, 'units', unit.id);
